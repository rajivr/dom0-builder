	...
	if [ ! -e "$WORKDIR/${_dir}" ]; then
		...
		if [ -z "$_simulate" ]; then
			...
			if build_${section} "$@"; then
				...
				# ViryaOS: Disable this so failure artifacts are available
				# rm -rf "$DESTDIR"
				_fail="yes"
				return 1
			fi
...
build_profile() {
	...
	# Cheat a bit here
	# NOTE: Unline x86_64, we do not have viryaos_grub_efi here.
	all_sections="kernels rootfs_qemu_aarch64"
